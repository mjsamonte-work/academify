...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Academify')" class="grid">
                ...
                @if(auth()->user()->can('students.view') || auth()->user()->can('guardians.view') || auth()->user()->can('teachers.view'))
                    <flux:sidebar.group :heading="__('Records')" class="grid">
                        ...
                        @endcan
                        @can('guardians.view')
                            <flux:sidebar.item icon="users" :href="route('records.guardians.index')" :current="request()->routeIs('records.guardians.*')" wire:navigate>
                                {{ __('Guardians') }}
                            </flux:sidebar.item>
                        @endcan
                        @can('teachers.view')
                ...
                @can('teachers.view_own')
                    <flux:sidebar.group :heading="__('Teacher')" class="grid">
                        ...
                        @can('attendance.view_own')
                            <flux:sidebar.item icon="layout-grid" :href="route('teacher.attendance.index')" :current="request()->routeIs('teacher.attendance.*')" wire:navigate>
                                {{ __('Attendance Entry') }}
                            </flux:sidebar.item>
                        @endcan
                        @can('grades.view_own')
